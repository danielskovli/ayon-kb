---
name: ayon-houdini
description: SideFX Houdini in Ayon — `ayon-houdini` addon menu (Set Frame Range, Update Houdini Vars, Template Builder), publishing via automatic `/out` ROPs, full product-type catalogue (PointCache, VDB, Static Mesh, HDA, Arnold/Mantra/Karma/Redshift/V-Ray ROPs, Redshift Proxy, Camera, Composite, Review, USD variants — Assembly/Groom/Look/Model/Render), the loader HDA system (Generic Loader, LOP Import, Load Shot — with `$F` and `<UDIM>` auto-tokens plus AYON Entity URI mode), `AYON_CONTAINERS` subnetwork wrapping, and the five Deadline render targets (Local, Use existing, Farm, Farm split export/render, Local Export & Farm). Use when writing Houdini creators/loaders, debugging a Houdini publish, or working with Ayon's USD contribution workflow.
when_to_use: Triggered by "Houdini", "ayon-houdini", "Generic Loader HDA", "LOP Import", "Load Shot", "AYON_CONTAINER", "Update Houdini Vars", "$HIP", "USD Assembly", "USD Groom", "USD Look", "USD Model", "USD Render", "Karma ROP", "Mantra ROP", "Redshift Proxy Houdini", "split export render", "AYON Entity URI".
---
