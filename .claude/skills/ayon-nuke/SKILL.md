---
name: ayon-nuke
description: Foundry Nuke in Ayon — `ayon-nuke` addon menu (Set Resolution / Frame Range / Colorspace / Apply All / Push to Project / Build Workfile), product types (Render, Prerender, Image/Plate, Review, Gizmo, Nukenodes), AYON-managed Write node convention (versionless until publish), publish stages (Collect→Validate→Extract→Integrate→Finalize), farm targets (Local, Use existing frames, On Farm, workfile-as-Deadline-dependency), Templated Workfile Builder with Create/Load placeholder nodes, colour management via `imageio` addon, commercial-licence-only caveat. Use when writing Nuke creators/loaders, debugging publish, helping an artist, or configuring Nuke templates.
when_to_use: Triggered by "Nuke", "ayon-nuke", "Write node Ayon", "versionless render Nuke", "Prerender", "Nuke Gizmo", "Nukenodes", "Templated Workfile Builder", "PLACEHOLDER node", "imageio", "Nuke colour management", "Push to Project", "workfile dependency Deadline", "Nuke commercial licence", "Nuke 14".
---
