---
name: ayon-recipes
description: Ready-made Ayon developer recipes — concrete "I want to build X → do Y" patterns. Use when starting an implementation task that matches a common pattern, or when user says "I want to build", "how do I scaffold", "I need to react to event X", "add a button that runs on the artist's machine", "new product type", "validator with repair", "download outside DCC", "expose REST endpoint", "frontend tab in Ayon UI", "host-scoped setting", "CI for my addon", "ship to Deadline", "test my addon". Covers: scaffold a new addon, server UI action / button that runs client CLI, event reactions, new product types, self-repairing validators, downloading representations outside a DCC, adding frontend scopes, custom REST endpoints, host-scoped settings, CI packaging, running scripts inside DCC env, launch-env injection, colorspace in publish, Deadline farm submission, local addon testing.
---
